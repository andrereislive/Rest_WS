...
def detectLabel(outDict, classes):
...
def writeDetectionsFile(outPath, outDict, classes):
    outTable = detectLabel(outDict,classes)
...
def writeStringToFile(filePath, stringContent):   
    # pass the filePath complete Ex. C:/Files/myFile.json or txt or whatever    
    text_file = open(filePath, "w")
    text_file.write(stringContent)
    text_file.close()
...
def switchLabelIntToRealName(label, classes):
    ...
    labelsVar = [None] * len(label) 
    for index in range(len(label)):
        labelsVar[index] = classes[label[index]]
    return labelsVar   
    
# minConfidence has to be float ex 1.1
# jsonObject has to be in this format '[{"top": "0", "bottom": "712", .. },{"top": "0", ...}]'
def applyPrecisionToJSon(minConfidence,jsonObject):
    array_delete = [] 
    for key_array in jsonObject:
        
        for attr_key in key_array:
            value = key_array[attr_key]
            if attr_key == "score" and float(key_array[attr_key]) < minConfidence:
                array_delete.append(key_array) 

    for x in array_delete:
        if(x != None):
            jsonObject.remove(x)

    return  jsonObject   

# Refina as rois que estao uma por cima da outra em um mesmo objeto, retornando entao uma roi que cobre o objeto
def refineRois(roiLabels, roiScores, roiRelCoords, classes,
                     nmsKeepIndices = None, boDrawNegativeRois = True, boDrawNmsRejectedRois = True,
                     decisionThreshold = 0.0):
    array_return = []                 
    # read and resize image
    ...
    for iter in range(0,3):
        ...
        for roiIndex in range(len(roiRelCoords)):
            ...
            if iter == 2 and label > 0:
                if not nmsKeepIndices or (roiIndex in nmsKeepIndices):
                    font = ImageFont.truetype("arial.ttf", 18)
                    text = classes[label]
                    array_return.append(roiIndex)

    print(array_return) 
    indexes = []
    for itx in range(len(roiRelCoords)):
       
        if itx in array_return:
            pass
        else:
            indexes.append(itx)
        
    roiRelCoords = np.delete(roiRelCoords, indexes,0)
    roiLabels = np.delete(roiLabels, indexes,0)
    roiScores = np.delete(roiScores, indexes,0)

          
    return roiLabels,  roiScores,  roiRelCoords




def removeNonMaximaSuppressionJSon(jsonObject):
    array_delete = [] 
    for key_array in jsonObject:
        ...
        for attr_key in key_array:
            value = key_array[attr_key]
            if attr_key == "nms":
                print(str(key_array[attr_key]))
                if str(key_array[attr_key]) == "True":
                    array_delete.append(key_array) 
